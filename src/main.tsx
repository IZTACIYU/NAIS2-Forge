...
async function performAutoBackup() {
    try {
        const lastBackupStr = localStorage.getItem('nais2-forge-last-auto-backup')
        ...
        localStorage.setItem('nais2-forge-last-auto-backup', now.toString())
...
async function checkDataIntegrity(): Promise<boolean> {
    try {
        // 중요 스토어들의 데이터 확인
        const criticalStores = ['nais2-forge-character-prompts', 'nais2-forge-scenes', 'nais2-forge-presets']
        ...
        for (const storeKey of criticalStores) {
            ...
            try {
                ...
                if (storeKey === 'nais2-forge-character-prompts') {
                    const presetCount = state?.presets?.length || 0
                    const charCount = state?.characters?.length || 0
                    
                    // 이전 기록과 비교
                    const prevStats = localStorage.getItem('nais2-forge-integrity-character-prompts')
                    if (prevStats) {
                    ...
                    // 현재 통계 저장
                    localStorage.setItem('nais2-forge-integrity-character-prompts', JSON.stringify({ presets: presetCount, characters: charCount }))
                }
                
                // scenes 체크
                if (storeKey === 'nais2-forge-scenes') {
                    const presetCount = state?.presets?.length || 0
                    const totalScenes = state?.presets?.reduce((sum: number, p: { scenes?: unknown[] }) => sum + (p.scenes?.length || 0), 0) || 0
                    
                    const prevStats = localStorage.getItem('nais2-forge-integrity-scenes')
                    if (prevStats) {
                        const prev = JSON.parse(prevStats)
                    ...
                    localStorage.setItem('nais2-forge-integrity-scenes', JSON.stringify({ presets: presetCount, scenes: totalScenes }))
                }
            } catch (parseErr) {
    ...
    if (!isDbInitFailed()) {
        ...
        try {
            await migrateIndexedDBKeys([
                ['nais-library-storage', 'nais2-forge-library'],  // Library items (was already IndexedDB)
                ['tools-storage', 'nais2-forge-tools'],           // Tools settings
                ['nais-update', 'nais2-forge-update'],            // Update state
            ])
        ...
        try {
            await migrateFromLocalStorage([
                'nais2-forge-generation',        // Main mode prompts & settings (CRITICAL!)
                'nais2-forge-character-store',   // Character/Vibe images
                'nais2-forge-character-prompts', // Character prompt presets (CRITICAL - 100+ presets)
                'nais2-forge-presets',           // Generation presets
                'nais2-forge-settings',          // App settings
                'nais2-forge-auth',              // Auth tokens
                'nais2-forge-scenes',            // Scene mode data (CRITICAL)
                'nais2-forge-shortcuts',         // Keyboard shortcuts
                'nais2-forge-theme',             // Theme settings
                'nais2-forge-wildcards',         // Wildcard/Fragment data
                'nais2-forge-layout',            // Layout preferences
                'nais2-forge-library',           // Library items
                'nais2-forge-tools',             // Tools settings (brush size, etc.)
                'nais2-forge-update',            // Update state
            ])
        ...
        cleanupLargeData('nais2-forge-wildcards', 100).then((cleaned) => {
            if (cleaned) {
