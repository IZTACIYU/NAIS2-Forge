...
export function FragmentPromptDialog({ open, onOpenChange, embedded = false }: FragmentPromptDialogProps) {
    ...
    const reorderFolders = useFragmentStore(state => state.reorderFolders)
    ...
    const handleDragEnd = (event: DragEndEvent) => {
        ...
        if (activeId.startsWith('folder:') && overId.startsWith('folder:')) {
            const oldIndex = folders.findIndex(f => `folder:${f}` === activeId)
            const newIndex = folders.findIndex(f => `folder:${f}` === overId)
    ...
    const content = (
        <div className="h-full flex flex-col p-4">
                <div className="flex items-center gap-2 pb-4">
                    <div className="flex items-center gap-2 min-w-0 flex-1 font-semibold text-lg">
                        ...
                        <ScrollArea className="flex-1">
                            ...
                            >
                                <div className="p-2">
                                    ...
                                    {/* ??? */}
                                    <SortableContext
                                        items={folders.map(folder => `folder:${folder}`)}
                                        strategy={verticalListSortingStrategy}
                                    >
                                        {folders.map(folder => (
                                            <FolderItem
