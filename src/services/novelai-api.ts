...
const API_ENDPOINTS = {
    // Primary API
    // 2026-06 NAI 엔드포인트 이전: user 계열이 old API domain에서 image.novelai.net으로 이동함
...
export async function generateImage(
    ...
    try {
        ...
        const apiParameters = {
            ...
            v4_prompt: {
                caption: {
                    base_caption: removePromptComments(params.prompt),
            ...
            v4_negative_prompt: {
                caption: {
                    base_caption: removePromptComments(params.negative_prompt),
                    char_captions: [] as { char_caption: string, centers: { x: number, y: number }[] }[],
                },
                legacy_uc: false,
            },
            ...
            for (const char of params.characterPrompts) {
                if (char.enabled && char.prompt.trim()) {
                    ...
                    apiParameters.v4_prompt.caption.char_captions.push({
                        char_caption: removePromptComments(char.prompt),
                        centers: centers
                    })
