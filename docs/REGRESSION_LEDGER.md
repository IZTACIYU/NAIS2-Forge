...
## R-036 — 전체보기의 상세 전환은 격자 위치를 보존한다

- **Date:** 2026-09-03
- **Area:** scene review tabs, all-images grid
- **Symptom/Risk:** 전체보기에서 이미지를 열 때 개별검수 탭으로 이동하거나 격자를 제거하면, 돌아왔을 때 사용자가 보던 스크롤 위치를 잃는다.
- **Root cause:** 탭 전환과 화면 전환을 같은 상태로 처리하면 전체보기 격자의 스크롤 컨테이너가 unmount된다.
- **Invariant to preserve:** 전체보기에서 연 상세 화면은 활성 탭을 바꾸지 않으며, 격자 스크롤 컨테이너를 숨기기만 하고 유지한다.
- **Fix:** 활성 탭과 전체보기 내부 상세 상태를 분리하고, 상세 표시 중에도 격자를 mount 상태로 유지한다.
- **Regression coverage:** 전체보기 중간까지 스크롤한 뒤 이미지를 열고 돌아와 같은 위치가 유지되는지 확인한다.
- **Do not "fix" by:** 전체보기 이미지 클릭을 개별검수 탭 선택으로 처리하거나 상세 화면에서 격자 DOM을 제거하기.
