...
- **Regression coverage:** `node --experimental-strip-types scripts/check-share-card-metadata.mjs`에서 공식/Forge PNG 메타데이터의 동시 왕복을 검사한다. 카드에는 원본 이미지를 다시 그리지 않고 프롬프트와 파라미터만 세로로 표시하며, 원본에 생성 메타데이터가 없으면 저장을 막는다.
