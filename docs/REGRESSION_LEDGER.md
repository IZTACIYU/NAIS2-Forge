...
- **Symptom/Risk:** 개별검수 우측 히스토리에 기존 씬 이미지가 섞이거나, 검수창에서 만들지 않은 이미지가 추가되거나, 임시 목록이 다음 실행까지 남을 수 있다.
- **Root cause:** 저장된 `scene.images`를 검수 히스토리의 원본으로 사용하면 생성 출처와 검수 세션 경계를 구분할 수 없다.
- **Invariant to preserve:** 검수 히스토리는 검수창에서 생성 요청을 시작한 씬의 런타임 완료 이벤트만 수집하고, 검수창을 닫으면 폐기한다. 저장된 씬 이미지 목록은 검수 대상 선택에만 사용한다.
- **Fix:** 씬 생성 완료 이벤트에 프리셋·씬 식별자를 포함하고 검수창이 추적 중인 씬만 컴포넌트 로컬 히스토리에 추가한다.
- **Regression coverage:** `npm run check:scene-review-generation`에서 일치하는 프리셋·씬·경로만 수집 대상으로 판정되는지 검사한다.
- **Do not "fix" by:** 임시 히스토리를 persisted store에 추가하거나 `scene.images` 전체를 우측 히스토리로 다시 표시하기.
