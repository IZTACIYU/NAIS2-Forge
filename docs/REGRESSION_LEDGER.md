...
- **Invariant to preserve:** 검수를 시작하지 않은 경우 각 씬에서는 최신 즐겨찾기 이미지 한 장을 우선하고, 즐겨찾기가 없으면 최신 이미지 한 장을 선택한다. 검수 판정을 시작한 경우에는 통과한 정확한 이미지만 ZIP·Cloudflare 대상으로 사용한다.
- **Fix:** 기본 대표 이미지 선택 함수는 세 경로가 함께 사용하고, 검수 판정이 생긴 실행 세션에서는 공통 검수 큐 필터가 출력 대상을 제한한다.
